...
# read_artifact caps: a transcript or a translation is the point of the tool,
# a two-hour subtitle file dumped whole into a prompt is not. Text is cut to
# a character budget; JSON documents are cut by their `segments`/`chunks`
# list, which is what makes them long.
MAX_ARTIFACT_CHARS = 20_000
DEFAULT_ARTIFACT_ITEMS = 60
MAX_ARTIFACT_ITEMS = 300
# Deliverables and pipeline documents the assistant has any business reading.
# Media is excluded: it cannot interpret a wav, and the byte count would be
# the whole budget.
READABLE_ARTIFACT_SUFFIXES = frozenset(
    {".json", ".txt", ".md", ".srt", ".vtt", ".ass", ".csv", ".html"}
)
...
instead of guessing about the state of the system. When a question is \
about what a task actually produced rather than its shape -- how a line \
was translated, what the proofreader flagged, who a segment was assigned \
to -- open the file: list_artifacts shows what a task has, and \
read_artifact returns the contents of transcripts, translations, review \
notes and text deliverables. You can also set up \
...
read_logs、run_preflight）蒐集事實，不要猜測系統狀態。問題若涉及任務實際\
產出的內容而非它的結構——某句話怎麼翻的、校對標了什麼、某段指給哪個說話人\
——就把檔案打開：list_artifacts 列出任務有哪些產物，read_artifact 讀出逐字\
稿、譯文、校對／品質標註與文字成品的內容。你也可以替操作者準備\
...
推測で語らないでください。タスクの構造ではなく実際の中身についての質問\
——ある行がどう訳されたか、校正で何が指摘されたか、どの区間が誰の発話か\
——であれば、ファイルを開いてください。list_artifacts でタスクの成果物を\
一覧し、read_artifact で文字起こし・訳文・校正／品質メモ・テキスト成果物の\
中身を読み取れます。オペレーターの依頼があれば作業の準備もできます：\
...
def build_assistant_tools(ws: Workspace) -> list[AgentTool]:
    ...
    def _task_dir_or_error(project: str, task_id: str) -> Path:
        try:
            ws.store.load(project, task_id)
        except FileNotFoundError:
            raise ToolError(f"task not found: {project}/{task_id}") from None
        return ws.store.task_dir(project, task_id)

    def list_artifacts(args: dict) -> str:
        project = str(args["project"])
        task_id = str(args["task_id"])
        store = ArtifactStore(_task_dir_or_error(project, task_id))
        items = [
            {
                "file": item["file"],
                "name": item["name"],
                "size": item["size"],
                "readable": Path(item["file"]).suffix.lower()
                in READABLE_ARTIFACT_SUFFIXES,
            }
            for item in store.list_artifacts()
        ]
        return json.dumps(items, ensure_ascii=False)

    def read_artifact(args: dict) -> str:
        project = str(args["project"])
        task_id = str(args["task_id"])
        name = str(args["name"])
        # Name resolution stays inside the artifacts directory: the tool takes
        # an artifact name, never a path, so a traversal attempt resolves to
        # nothing rather than to a file elsewhere on disk.
        if "/" in name or "\\" in name or name in {"", ".", ".."}:
            raise ToolError(f"not an artifact name: {name}")
        if Path(name).suffix.lower() not in READABLE_ARTIFACT_SUFFIXES:
            raise ToolError(
                f"{name} is not a readable artifact kind; readable kinds are "
                + ", ".join(sorted(READABLE_ARTIFACT_SUFFIXES))
            )
        store = ArtifactStore(_task_dir_or_error(project, task_id))
        try:
            path = store.latest_path(name) if "-" not in name[:3] else store.dir / name
        except FileNotFoundError as error:
            raise ToolError(str(error)) from None
        if not path.is_file():
            raise ToolError(f"no artifact {name} in {project}/{task_id}")

        raw = args.get("items", DEFAULT_ARTIFACT_ITEMS)
        try:
            limit = max(1, min(MAX_ARTIFACT_ITEMS, int(raw)))
        except (TypeError, ValueError):
            raise ToolError("items must be an integer") from None

        text = path.read_text(encoding="utf-8", errors="replace")
        if path.suffix.lower() != ".json":
            body = text[:MAX_ARTIFACT_CHARS]
            return json.dumps(
                {
                    "file": path.name,
        ...
        try:
            ...
            raise ToolError(f"{path.name} is not valid JSON: {error}") from None
        truncated = False
        if isinstance(data, dict):
            for key in ("segments", "chunks", "flags", "speakers"):
                value = data.get(key)
                if isinstance(value, list) and len(value) > limit:
                    data = {**data, key: value[:limit]}
                    truncated = True
        elif isinstance(data, list) and len(data) > limit:
            data, truncated = data[:limit], True
        return json.dumps(
            {"file": path.name, "truncated": truncated, "content": data},
            ensure_ascii=False,
        )
    ...
    return [
        ...
        AgentTool(
            name="list_artifacts",
            description=(
                "List a task's artifacts (file, name, size) and whether each "
                "one's contents can be read with read_artifact."
            ),
            parameters={
                "project": {"type": "string", "required": True, "description": "task's project"},
                "task_id": {"type": "string", "required": True, "description": "task id"},
            },
            handler=_safe(list_artifacts),
        ),
        AgentTool(
            name="read_artifact",
            description=(
                "Read one artifact's contents: transcripts and translations "
                "(asr.json, translation.json), review notes "
                "(proofread-report.json, qc.json), speaker assignments "
                "(speakers.json), and text deliverables (.srt, .txt, .md). "
                "Pass the bare name for the latest version, or the prefixed "
                "file name for a specific one. Long documents come back "
            ...
            parameters={
                ...
                "name": {
                    ...
                    "required": True,
                    "description": (
                        "artifact name (translation.json) or file name "
                        "(06-translation.json); see list_artifacts"
                    ),
                },
                "items": {
                    "type": "integer",
                    "required": False,
                    "description": (
                        "how many segments/chunks/flags to return from a JSON "
                        "document, clamped to [1, 300]"
                    ),
                },
            },
            handler=_safe(read_artifact),
        ),
        AgentTool(
            name="budget_status",
