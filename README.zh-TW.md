...
桌面端的自動翻譯工作站。輸入影片、音訊、字幕或文件，由管線完成語音辨識、翻譯、校對、配音與匯出。
...
它是既有工具之上的編排層：媒體處理交給 ffmpeg，語音辨識交給 faster-whisper 或雲端引擎，翻譯接任何 OpenAI 相容端點、Anthropic 或 Gemini。Traduko 負責把這些串成可暫停、可續跑、花費可控的任務。
...
| 功能 | 說明 |
| --- | --- |
| 多表名詞表 | 各表綁定單一任務域或通用，支援分類與 CSV/JSON 匯入匯出。任務可複選全域表並疊加任務專屬表。 |
| ASR 偏置 | 名詞表同時偏置語音辨識：支援的引擎注入提示，其餘可插入輕量校對階段。修改後可對既有任務重新套用。 |
| Agent 校對 | 校對是帶工具的 agent 迴圈，可查詢名詞表與前後文，分多輪修訂譯文。強度可設定；預算中途用盡時保留目前最佳版本。 |
...
| 功能 | 說明 |
| --- | --- |
| 內建助理 | 可查詢任務狀態、預算、設定、日誌與預檢結果，並讀取逐字稿、譯文、標註等產物內容。設定變更只能提出待審提案，由操作者核准後才生效。 |
...
<td width="50%"><img src="docs/screenshot-outputs-light.png" alt="產出列表" /><br /><sub>產出依類型分組，音訊就地播放，字幕檔就地預覽並可調字級。</sub></td>
</tr>
<tr>
...
<td><img src="docs/screenshot-assistant-dark.png" alt="內建助理" /><br /><sub>內建助理將設定變更整理成可核准的 diff。</sub></td>
</tr>
<tr>
